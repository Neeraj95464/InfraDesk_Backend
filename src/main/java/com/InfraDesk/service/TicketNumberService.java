...
//        for (int attempt = 0; attempt < maxRetries; attempt++) {
//            try {
//                // ---- 1. Lock existing counter row ----
//                String sql = "SELECT id, last_seq FROM ticket_counters " +
//                        "WHERE company_id = (SELECT id FROM companies WHERE public_id = :companyPid) " +
//                        (departmentPublicId == null
//                                ? "AND department_id IS NULL "
//                                : "AND department_id = (SELECT id FROM departments WHERE public_id = :deptPid) ") +
//                        "FOR UPDATE";
//
//                Query q = em.createNativeQuery(sql);
//                q.setParameter("companyPid", companyPublicId);
//                if (departmentPublicId != null) {
//                    q.setParameter("deptPid", departmentPublicId);
//                }
//
//                @SuppressWarnings("unchecked")
...
//
//                if (rows.isEmpty()) {
//                    // ---- 2. Resolve entities from publicId ----
...
//                                        "select d from Department d where d.publicId = :pid and d.company.publicId = :cid",
//                                        Department.class)
//                                .setParameter("pid", departmentPublicId)
//                                .setParameter("cid", companyPublicId)
//                                .getSingleResult();
//                    }
//
//                    // ---- 3. Insert initial counter row ----
//                    TicketCounter newCounter = TicketCounter.builder()
//                            .company(company)
